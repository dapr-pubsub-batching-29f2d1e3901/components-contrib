	...
	if isBatchSubscribe {
		if consumer.k.consumeRetryEnabled {
			if err := retry.NotifyRecover(func() error {
				...
				consumer.k.logger.Errorf("Too many failed attempts at processing Kafka message: %s/%d/%d [key=%s]. Error: %v.", messages[0].Topic, messages[0].Partition, messages[0].Offset, asBase64String(messages[0].Key), err)
			}
		} else {
